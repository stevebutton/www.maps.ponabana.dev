...
class PT_Content_Views_Pro {
	 ...
	 * @var      string
	 */
	protected $plugin_slug = PT_CV_DOMAIN_PRO;
	...
	private function __construct() {
		...
		add_action( 'wp_head', array( 'PT_CV_Html_Pro', 'init_view_data' ) );
	}
	...
	private static function single_activate() {
		...
		self::request_sellwire( 'activate' );
	}
	...
	private static function single_deactivate() {
		...
		self::request_sellwire( 'deactivate' );
	}
	 ...
	 * @param string $action
	 */
	public static function request_sellwire( $action ) {
		if ( empty( $action ) ) {
			return '';
		}
		...
		$data['license'] = isset( $pt_cv_options['license_key'] ) ? $pt_cv_options['license_key'] : '';
		// CV-PRO file ID
		$data['file'] = 'Am';

		// Send the request & save response to $resp
		$content = @file_get_contents( sprintf( 'http://app.sellwire.net/api/1/%s?%s', $action . '_license', http_build_query( $data ) ) );
		if ( $content !== FALSE ) {
			$data = json_decode( $content, TRUE );
			if ( isset( $data['license'] ) ) {
				if ( $data['license'] == 'activated' ) {
					update_option( 'pt_cv_' . 'pro_lcs', 1 );
				}
				if ( $data['license'] == 'deactivated' ) {
					delete_option( 'pt_cv_' . 'pro_lcs' );
				}
			}
		}
	}
	...
	public function load_plugin_textdomain() {
		...
		// Load translation file in wp-content/languages/content-views-pro/
